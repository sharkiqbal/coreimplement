...
const ToolBadge = ({ name, className = "" }) => {
...
// ---------------------------------------------------------------------------
// Section 1: Smart Document Processing (AI Intake + AI Document Processing)
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-200">
        <h2 className="font-bold text-slate-900">Smart Document Processing</h2>
        <p className="text-sm text-slate-500 mt-0.5">
          AI Intake + AI Document Processing — a document arrives, gets read,
          and either posts itself automatically or gets flagged for a human.
        </p>
      </div>
...
// ---------------------------------------------------------------------------
// Section 2: Company Knowledge Assistant
// ---------------------------------------------------------------------------
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-200">
        <h2 className="font-bold text-slate-900">
          Ask the Firm's Knowledge Base
        </h2>
        <p className="text-sm text-slate-500 mt-0.5">
          RAG over internal SOPs and past client work, with citations — so
          staff get answers instead of digging through folders.
        </p>
      </div>
          ...
          <div className="space-y-4 mb-5 max-h-96 overflow-y-auto">
            {messages.map((m, i) => (
              ...
              >
                ...
                >
                  <p>{m.text}</p>
                  {m.source && (
                    <div className="flex items-center gap-1.5 mt-2 pt-2 border-t border-slate-200 text-xs text-slate-400">
                      <BookOpen className="w-3 h-3" />
                      Source: {m.source}
                      <ToolBadge name="Google Drive" className="ml-1" />
                    </div>
                  )}
                </div>
                {m.role === "user" && (
            ...
            {thinking && (
              <div className="flex gap-3">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-400 flex items-center gap-2">
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {KNOWLEDGE_QA.map((qa) => (
            <button
              ...
              onClick={() => ask(qa)}
              disabled={thinking}
              className="text-xs font-medium px-3 py-1.5 bg-slate-50 hover:bg-blue-50 hover:text-blue-700 border border-slate-200 hover:border-blue-200 rounded-full transition-colors disabled:opacity-50"
            >
              {qa.question}
            </button>
          ))}
...
// ---------------------------------------------------------------------------
// Section 3: AI Workflow Manager
// ---------------------------------------------------------------------------

const WorkflowManagerDemo = () => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between gap-4">
        <div>
          <h2 className="font-bold text-slate-900">Daily Workflow Manager</h2>
          <p className="text-sm text-slate-500 mt-0.5">
...
const PersonaWorkspace = () => {
  const { industry, persona, industrySlug } = useOutletContext();

  return (
    <div className="space-y-6">
      <SEO
        title={`${persona.name} AI Workspace`}
        description="Sample product dashboard preview."
        path={`/platform-preview/${industrySlug}/workspace`}
        noindex
      />

      <div className="flex items-start gap-3 bg-gradient-to-br from-blue-50 to-indigo-50 border border-indigo-100 rounded-xl p-4">
        <Sparkles className="w-5 h-5 text-indigo-600 flex-shrink-0 mt-0.5" />
        <p className="text-sm text-slate-700">
          A live look at how AI would handle {persona.name}'s daily
          operations. Everything below is interactive — try it.
        </p>
      </div>
      ...
      {industry === "Accounting" ? (
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-10 text-center">
          ...
          <p className="font-bold text-slate-900 mb-1">
            This workspace is being tailored for {industry}
          </p>
